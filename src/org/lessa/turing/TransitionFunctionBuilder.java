...
public class TransitionFunctionBuilder {
	...
	private final static String ERR_ARRAY_SIZE_MISMATCH = "The '%s' array must have the same number of symbols as tapes. Found %d but expected %d.";
	private final int numTapes;

	private Map<Key, Transition> transitionMap;
	...
	private Transition buildTransition(final Character[] symbolsToWrite, final Move[] tapeMoves, final String state) {
	...
	public TransitionFunctionBuilder withTranstion(String currentState, Character[] currentTapeSymbols,
	      Character[] symbolsToWrite, Move[] tapeMoves, String newState) {

		if (currentState == null || currentState.length() == 0) {
			throw new IllegalArgumentException("The 'currentState' argument must be a non-empty string.");
		}

		if (currentTapeSymbols == null || currentTapeSymbols.length != numTapes) {
			throw new IllegalArgumentException(String.format(ERR_ARRAY_SIZE_MISMATCH, "currentTapeSymbols",
		...
		if (tapeMoves == null || tapeMoves.length != numTapes) {
			throw new IllegalArgumentException(
			      String.format(ERR_ARRAY_SIZE_MISMATCH, "tapeMoves", tapeMoves == null ? 0 : tapeMoves.length, numTapes));
		}
