...
import org.lessa.turing.Event;
import org.lessa.turing.Transition;
...
final class LoggingEventHandler implements Event.EventHandler {
	@Override
	...
	private void logInternalState(long stateId, String state, List<Character> tapeHeadSymbols) {
		final String logEntry = String.format("{\"stateId\": %d, \"state\": \"%s\", \"tapeHeadSymbols\": %s}", stateId,
		      state, Arrays.toString(tapeHeadSymbols.toArray()));
		System.out.println(logEntry);
	}

	private void logTransition(long stateId, Transition transition) {
		final String logEntry = String.format(
		      "{\"stateId\": %d, \"transition\": {\"nextState\": \"%s\", \"writeToTapes\": %s, \"moves\": %s}}", stateId,
		      transition.state(), Arrays.toString(transition.outputs().toArray()),
