...
public final class Tokenizer implements ListIterator<Token> {
	...
	private boolean isSupportedSymbol(final int codePoint) {
		return codePoint == DOT || codePoint == EQUALS || codePoint == LAMBDA || codePoint == OPENING_PARENS
		      || codePoint == CLOSING_PARENS;
	}
		...
		for (int index = 0; index < buffer.length(); index++) {
			...
			if (Character.isWhitespace(buffer.codePointAt(index))) {
				...
				if (tokenStart != -1) {
					...
					result.add(new Token(TK_DEF.token().equals(token) ? TokenClass.DEF : TokenClass.NAME, token));
					tokenStart = -1;
				}
			}
